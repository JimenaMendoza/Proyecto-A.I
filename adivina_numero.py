...
import pickle
import subprocess
print('!hola¡ Soy Kanon ¿Cual es tu nombre?')
nombre = input()
def ns(c):
    while c!=("s") and c!=("n"):
...
while True:
    ...
    input('Bueno\t'+nombre+'\nPara iniciar preciona Enter ')
    print(''+nombre+'\tADIVINA El NUMERO-SUPER DESAFIO')
    input('En este juego\t'+nombre+' has de adivinar un número,escogido al azar por mi , dentro de un rango determinado.')
    print("")
...
NIVEL 4: ENTRE 0 Y 100000""")
    ...
    while tu_numero!=numero_elegido:
        ...
        if abs(tu_numero-numero_elegido)>0:#????
            if tu_numero!=num_anterior:
                ...
                repes+=1
                print(''+nombre+'\tHAS INTRODUCIDO EL MISMO NÚMERO DOS VECES SEGUIDAS')
    ...
    if tu_numero==numero_elegido:#SI EL USUARIO ACIERTA...
        print("¡BINGO!")
        print('Lo lograste '+nombre+' en',intentos,sing_plu(intentos))
        ...
        if marca[posi_marca]==1:#SI SE ACIERTA A LA PRIMERA LA MARCA VUELEVE A SU VALOR ORIGINAL
            ...
            pickle.dump(marca,open("mejor_marca","wb"))
    conti=ns(input('¿'+nombre+'\tQuieres Jugar otra vez?: '))
    if conti==("n"):
        break
    try:
